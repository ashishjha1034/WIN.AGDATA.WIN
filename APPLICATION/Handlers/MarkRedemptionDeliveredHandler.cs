...
public class MarkRedemptionDeliveredHandler : IRequestHandler<MarkRedemptionDeliveredCommand, RedemptionDto>
{
    private readonly IRedemptionRepository _redemptionRepo;
    private readonly IUnitOfWork _uow;

    public MarkRedemptionDeliveredHandler(IRedemptionRepository redemptionRepo, IUnitOfWork uow)
    {
        _redemptionRepo = redemptionRepo;
        _uow = uow;
    }

    public async Task<RedemptionDto> Handle(MarkRedemptionDeliveredCommand request, CancellationToken cancellationToken)
    {
        ...
        redemption.MarkDelivered(request.DeliveredBy);
