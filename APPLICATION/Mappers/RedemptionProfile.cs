...
public class RedemptionProfile : Profile
{
    public RedemptionProfile()
    {
        ...
        CreateMap<Redemption, RedemptionDetailDto>()
            ...
            .ForMember(dest => dest.UserCurrentBalance, opt => opt.MapFrom(src => src.User.PointsAccount.CurrentBalance))
            .ForMember(dest => dest.UserTotalEarned, opt => opt.MapFrom(src => src.User.PointsAccount.TotalEarned))
            .ForMember(dest => dest.UserTotalRedeemed, opt => opt.MapFrom(src => src.User.PointsAccount.TotalRedeemed))
            // Product snapshot
            ...
            .ForMember(dest => dest.ProductPointsPerUnit, opt => opt.MapFrom(src => src.Product.CurrentPricing))
            .ForMember(dest => dest.ProductTotalPoints, opt => opt.MapFrom(src => src.PointsSpent));
    }
}
