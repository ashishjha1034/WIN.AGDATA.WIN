...
public class DeactivateUserHandlerTests
{
    ...
    public async Task Handle_WhenActiveEventRegistrationsExistWithForce_StillBlocked()
    {
        ...
        _eventRepositoryMock
            .Setup(r => r.GetUserEventRegistrationStatsAsync(targetUserId))
            .ReturnsAsync((2, 0)); // 2 ACTIVE event registrations should block even with Force
    ...
    public async Task Handle_WhenMultipleBlockingConditionsExist_ReturnsAllReasons()
    {
        ...
        _eventRepositoryMock
            ...
            .ReturnsAsync((1, 0)); // 1 active event registration
    ...
    public async Task Handle_WhenCompletedEventsExist_ReturnsWarningsWithoutForce()
    {
        ...
        // SetupSoftWarningsData sets event stats to (0, completedEvents)
    ...
    {
        // Generate a 9-character employee ID (EMP + 6 chars from GUID)
        var empId = $"EMP{userId.ToString()[..6].ToUpper()}";
        
        var user = new User(
            EmployeeId.Create(empId),
    ...
    {
        // Event stats include completed events - use the parameter value!
        _eventRepositoryMock
            .Setup(r => r.GetUserEventRegistrationStatsAsync(userId))
            .ReturnsAsync((0, completedEvents)); // (activeCount, completedCount)
