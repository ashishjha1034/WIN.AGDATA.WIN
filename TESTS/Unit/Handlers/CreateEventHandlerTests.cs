...
public class CreateEventHandlerTests
{
    ...
    private const string ValidDescription = "This is a comprehensive test event description with sufficient length for validation requirements.";
    ...
    public async Task Handle_WhenRegistrationEndAfterEventStart_ThrowsValidationException()
    {
        ...
        Assert.Equal("RegistrationEndDateUtc", exception.FieldName);
        Assert.Contains("must be strictly earlier than event date", exception.Message);
    }
    ...
    [Fact]
    public async Task Handle_WhenRegistrationEndInPast_ThrowsValidationException()
    {
        // Arrange: Past registration end (which is now rejected)
        var eventDate = DateTime.UtcNow.AddDays(7);
        ...
        // Act & Assert - Should reject past registration dates
        var exception = await Assert.ThrowsAsync<ValidationException>(
            () => _handler.Handle(command, CancellationToken.None));
